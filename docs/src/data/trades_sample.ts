/**
 * Generate 10K rows of sample trade data using DuckDB's generate_series.
 * Same schema as the full 1M-row trades table, but loads instantly.
 * Used by all documentation examples.
 */

export const TRADES_SAMPLE_SQL = `
CREATE TABLE IF NOT EXISTS trades_sample AS
WITH raw AS (
  SELECT
    i AS id,
    hash(i * 7 + 13) AS h1,
