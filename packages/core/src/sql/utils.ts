...
export function pickStringColumn(columns: { name: string; mappedType: string }[], skip?: string): string {
  const col = columns.find((c) => c.mappedType === 'string' && c.name !== skip && !isIdentityColumn(c.name));
