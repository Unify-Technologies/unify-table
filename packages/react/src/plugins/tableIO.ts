...
export interface TableIOHandle {
  exportCSV(options?: { selection?: boolean; delimiter?: string; headers?: boolean; filename?: string }): Promise<void>;
...
export function tableIO(): TablePlugin & { getHandle: (ctx: TableContext) => TableIOHandle } {
  ...
  const plugin: TablePlugin & { getHandle: (ctx: TableContext) => TableIOHandle } = {
    ...
    getHandle(ctx: TableContext): TableIOHandle {
      ...
      async function exportSource(format: 'csv' | 'json' | 'parquet', filename: string, selectionOnly?: boolean) {
      ...
      _handle = {
        ...
        async importFile(file: File, options = {}) {
          const tableName = options.tableName ?? file.name.replace(/\.[^.]+$/, '').replace(/[^a-zA-Z0-9_]/g, '_');
          ...
          await ctx.engine.execute(`INSTALL httpfs; LOAD httpfs;`.replace(/;/g, ';\n')).catch(() => {
          ...
          if (ext === 'csv') {
            ...
            const prefix = options.append ? `INSERT INTO ${qt}` : `CREATE OR REPLACE TABLE ${qt} AS`;
