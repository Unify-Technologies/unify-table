...
function exportSourceItems(ctx: TableContext, format: "csv" | "json" | "parquet"): MenuItem[] {
  ...
  const doExport = async (selOnly: boolean) => {
    ...
    if (selOnly && l.selection.selectedIds.size > 0) {
      ...
      await l.engine.execute(
        `CREATE TEMP TABLE ${quoteIdent(tmp)} AS SELECT * FROM ${quoteIdent(table)} WHERE ${quoteIdent(idCol)} IN (${idList})`,
      );
  ...
  const items: MenuItem[] = [
    {
      label: `All rows  ${fmt(live.totalCount)} \u00D7 ${totalCols}`,
      action: () => doExport(false),
    },
  ];
  if (hasSel) {
    items.push({
  ...
  if (live.filters.length > 0) {
    items.push({
      label: `Filtered  ${fmt(live.totalCount)} \u00D7 ${totalCols}`,
      action: () => doExport(false),
    });
  }
  return items;
    ...
    action: () => {
      ...
      if (selection.selectedCells.length > 0) {
        ...
        const body = [...byRow.entries()]
          ...
          .map(([idx]) => {
            const row = rows[idx];
            return row ? columns.map((c) => row[c.field] ?? "").join("\t") : "";
          })
          .join("\n");
    ...
    items.map((item, i) => {
      if (item.type === "separator") {
        return createElement("div", {
          key: `sep-${i}`,
          style: { height: 1, backgroundColor: colors.border, margin: "4px 8px" },
        });
      }
        ...
        [
          createElement(
            ...
            [
              createElement(
                "span",
                { key: "icon", style: { display: "flex", opacity: 0.5, width: 14, flexShrink: 0 } },
                item.icon ?? null,
              ),
              ...
              item.shortcut
                ? createElement(
                    "span",
                    { key: "sc", style: { opacity: 0.3, fontSize: "0.7em", marginLeft: 16 } },
                    item.shortcut,
                  )
                : null,
              hasChildren
                ? createElement(
                    "span",
                    { key: "arrow", style: { opacity: 0.4, marginLeft: 8 } },
                    "\u25B8",
                  )
                : null,
            ],
