...
function StatsRender({ config, sql, engine }: DisplayRenderProps<StatsDisplayConfig>) {
  ...
  if (isLoading && rows.length === 0)
    return <div className="utbl-display-loading">Loading stats...</div>;
          ...
          <div
            style={{
              ...
              background: primary
                ...
                : "color-mix(in srgb, var(--utbl-accent) 12%, transparent)",
      ...
      {numericCols.map((col) => {
        ...
        return (
          <div key={col.name}>
            ...
            {isSelected && field && (
              <div
                style={{ marginLeft: 22, marginTop: 4, display: "flex", gap: 4, flexWrap: "wrap" }}
              >
                {STAT_AGGS.map((agg) => (
                  <button
                    key={agg}
