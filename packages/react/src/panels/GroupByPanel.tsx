...
export function GroupByPanel({ columns, search, groupByCols, setGroupByCols, showTooltip, hideTooltip }: BuiltInPanelProps) {
  ...
  return (
    <div className="utbl-panel-section" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {/* Divider */}
      {available.length > 0 && (
        <div style={{ borderTop: '1px solid var(--utbl-border)' }} />
      )}
          ...
          <div className="utbl-space-y-sm">
            {available.map((col) => (
              <div
                ...
                style={{ cursor: 'grab' }}
              >
                <span style={{ flexShrink: 0, color: 'var(--utbl-text-muted)', fontSize: '0.55rem', lineHeight: 1, userSelect: 'none' }}>⠿</span>
                <GroupToggle
                  grouped={false}
...
function GroupToggle({ grouped, onToggle, showTooltip, hideTooltip }: {
  grouped: boolean;
  onToggle: () => void;
  ...
  return (
    <button
      onClick={(e) => { e.stopPropagation(); setHovered(false); hideTooltip(); onToggle(); }}
