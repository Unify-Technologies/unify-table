...
export const GroupRow = memo(function GroupRow({
  ...
  return columns.map((col, colIdx) => {
    ...
    const isActiveCellHere =
      activeCell?.rowIndex === virtualIndex && activeCell?.colIndex === colIdx;
    ...
    if (colIdx === 0) {
      ...
      return (
        <div
          key={col.field}
          className={`${styles.cell ?? ""} ${styles.groupRow ?? ""}`}
          style={{
            ...cellStyle,
            paddingLeft: density.px + depth * 20,
            display: "flex",
            alignItems: "center",
            gap: 6,
          }}
        >
          ...
          <span
            className="utbl-group-value"
            style={{ overflow: "hidden", textOverflow: "ellipsis", flexShrink: 1, minWidth: 0 }}
          >
            {formatValue(groupValue, fmt) || "\u2014"}
          </span>
          <span
            className="utbl-group-count"
            style={{ opacity: 0.5, fontSize: "0.75em", flexShrink: 0 }}
          >
    ...
    if (isGroupedCol) {
      return (
        <div
          key={col.field}
          className={`${styles.cell ?? ""} ${styles.groupRow ?? ""} utbl-cell-muted`}
          style={{ ...cellStyle, opacity: 0.25 }}
        >
          {"\u2014"}
        </div>
      );
    ...
    if (aggValue !== undefined && aggValue !== null) {
      const rawCellStyle =
        typeof col.cellStyle === "function" ? col.cellStyle(aggValue, row) : (col.cellStyle ?? "");
      const { className: dynClass, inlineStyle } = resolveCellStyle(rawCellStyle);
      return (
        <div
          key={col.field}
          className={`${styles.cell ?? ""} ${styles.groupRow ?? ""} ${dynClass}`}
