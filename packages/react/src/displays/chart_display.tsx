...
function ChartRender({ config, sql, engine }: DisplayRenderProps<ChartDisplayConfig>) {
  ...
  if (error) {
    return <div ref={ref} className="utbl-display-error">{error.message}</div>;
  }

  if (isLoading && rows.length === 0) {
    return <div ref={ref} className="utbl-display-loading">Loading chart data...</div>;
  }

  if (!option) {
    return <div ref={ref} className="utbl-display-loading">No data</div>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: '0.625rem' }}>
      ...
      <div>
        ...
        <select value={config.x} onChange={(e) => onChange({ ...config, x: e.target.value })} style={selectStyle}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 3 }}>
          ...
          <select
            ...
            onChange={(e) => onChange({ ...config, y: { ...yArr[0], agg: e.target.value as ValueField['agg'] } })}
      ...
      <div style={{ display: 'flex', gap: 3 }}>
        {([
          { key: 'stacked' as const, label: 'Stacked', icon: <Layers size={14} strokeWidth={2} /> },
          { key: 'horizontal' as const, label: 'Horizontal', icon: <Columns2 size={14} strokeWidth={2} /> },
