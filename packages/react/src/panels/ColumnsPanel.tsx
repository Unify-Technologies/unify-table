...
export function ColumnsPanel({
  ...
  const displayColumns = orderedFields
    ...
    .filter(
      (c): c is NonNullable<typeof c> =>
        c !== undefined && (c.label ?? c.field).toLowerCase().includes(search.toLowerCase()),
    );
  ...
  const handleReorder = useCallback(
    (reordered: DragListItem[]) => {
      ...
      const newOrder = [...nonDisplay.map((c) => c.field), ...reordered.map((c) => c.key)];
...
function EyeToggle({
  ...
  return (
    ...
    >
      {showClosed ? <EyeOff size={12} strokeWidth={2.5} /> : <Eye size={12} strokeWidth={2.5} />}
    </button>
  );
}
