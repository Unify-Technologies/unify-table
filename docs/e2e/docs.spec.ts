...
test.describe("Documentation Site Audit", () => {
  ...
  test("33 - Demo page loaded", async ({ page }) => {
    ...
    await page.locator('[data-index="0"], .utbl-row, canvas').first().waitFor({ timeout: 15000 });
  ...
  test("34 - Demo right-click context menu", async ({ page }) => {
    ...
    await page.locator(".utbl-row").first().waitFor({ timeout: 15000 });
  ...
  test("35 - Search dialog", async ({ page }) => {
    ...
    await page.locator('[role="dialog"], [class*="search"], input[type="search"], input[placeholder*="earch"]').first().waitFor({ timeout: 5000 });
